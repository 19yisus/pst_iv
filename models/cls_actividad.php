...
class cls_actividad extends cls_db {
    ...
    public function setDatos($datos){
        ...
        $this->descripcion=isset($datos['descripcion']) ? $this->Clean(intval($datos['descripcion'])) : null;
        $this->fecha_actividad=isset($datos['fecha_actividad']) ? $this->Clean(intval($datos['fecha_actividad'])) : null;
        $this->tiempo=isset($datos['tiempo']) ? $this->Clean(intval($datos['tiempo'])) : null;
    }
    ...
    function consultarUnaActividadDelProyecto(){
        $sqlConsulta = "SELECT * FROM actividad INNER JOIN asistencia_actividad ON asistencia_actividad.id_actividad=actividad.id_actividad where actividad.id_proyecto=$this->id_actividad AND actividad.id_proyecto=$this->id_actividad ;";
        $result = $this->Query($sqlConsulta);
        return $result;
    }
